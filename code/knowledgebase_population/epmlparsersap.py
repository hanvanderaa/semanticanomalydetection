...
from knowledgebase_population import pnmlexporter
...
def parse_EPML(file, output_path, pnml = True, jpg = True):
    ...
    for epc in epcs:
        ...
        # Create Petri Net
        net = PetriNet(f"Petri Net ({epc.attributes['name'].value})")
        net_name = "model_" + epc.attributes['name'].value
        ...
        if jpg == True:
            ...
            pn_vis_factory.save(gviz, f"{output_path}{net_name}.jpg")

        # Save as pnml
        if pnml == True:
            initial_marking = Marking()
            initial_marking[place_ids.get(0)] = 1
            pnmlexporter.export_pnml(net, initial_marking, f"{output_path}{net_name}.pnml")
...
def main():
    ...
    file = "../input/SAP/EPML/SAPModels.epml"
    parse_EPML(file, "../input/SAP/", True, True)
